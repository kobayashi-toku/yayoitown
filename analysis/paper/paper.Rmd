...
このセンテンスはRコードを入れられる。「`r 5 + 5`」。ここには5＋5が入っています。

長い式も入れられる。100*3が入っています。
```{r}
100 * 3
```

```{r iris-plot,fig.cap="アイリスの表"}
library(ggplot2)
...
```
この図はFigure \@ref(fig:iris-plot)。自動で図表番号が入ります。

6行のみ読み込み。
```{r iris-table}
my_date <- head(iris)
knitr::kable(my_date, caption = "アイリスの表")
```
この表はTable　\@ref(tab:iris-table)。やはり自動で番号が入ります。
